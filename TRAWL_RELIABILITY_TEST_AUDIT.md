...
- **M-03 remains partially fixed:** a deterministic qBittorrent 403 reauthentication contract still needs an injectable credential/reauthentication seam.
- **`ArrSetupViewModel` has no unit-test file at all.** Jellyfin and Seerr have `JellyfinSetupViewModelTests`/`SeerrSetupViewModelTests`; the Arr equivalent does not exist. `validateAndSave` is ~200 lines containing the Keychain rollback, the SwiftData `rollback()`, the cancellation restore and the Prowlarr singleton branch, and the only things exercising any of it are the two UI journeys — which drive the *edit* path exclusively.
