...
// MARK: - ArrServiceProfile Tests

struct ArrServiceProfileTests {

    @Test func initDefaultsIsEnabledToTrue() {
        let profile = ArrServiceProfile(
            displayName: "My Sonarr",
            hostURL: "http://192.168.1.1:8989",
            serviceType: .sonarr
        )
        #expect(profile.isEnabled == true)
    }

    @Test func initSetsDateAddedToNow() {
        let before = Date()
        let profile = ArrServiceProfile(
            displayName: "Test",
            hostURL: "http://localhost:7878",
            serviceType: .radarr
        )
        let after = Date()
        #expect(profile.dateAdded >= before)
        #expect(profile.dateAdded <= after)
    }

    @Test func initAssignsUniqueIDs() {
        let profile1 = ArrServiceProfile(displayName: "A", hostURL: "http://a", serviceType: .sonarr)
        let profile2 = ArrServiceProfile(displayName: "B", hostURL: "http://b", serviceType: .radarr)
        #expect(profile1.id != profile2.id)
    }

    @Test func apiKeyKeychainKeyFormat() {
        let profile = ArrServiceProfile(
            displayName: "Test",
            hostURL: "http://localhost",
            serviceType: .sonarr
        )
        let expectedKey = "arr_\(profile.id.uuidString)_apikey"
        #expect(profile.apiKeyKeychainKey == expectedKey)
    }

    @Test func apiKeyKeychainKeyContainsUUID() {
        let profile = ArrServiceProfile(
            displayName: "Test",
            hostURL: "http://localhost",
            serviceType: .prowlarr
        )
        #expect(profile.apiKeyKeychainKey.hasPrefix("arr_"))
        #expect(profile.apiKeyKeychainKey.hasSuffix("_apikey"))
        // UUID portion should be present (36 chars)
        let uuidPart = profile.apiKeyKeychainKey
            .dropFirst(4) // drop "arr_"
            .dropLast(7)  // drop "_apikey"
        #expect(UUID(uuidString: String(uuidPart)) != nil)
    }

    @Test func resolvedServiceTypeSonarr() {
        let profile = ArrServiceProfile(displayName: "X", hostURL: "http://x", serviceType: .sonarr)
        #expect(profile.resolvedServiceType == .sonarr)
    }

    @Test func resolvedServiceTypeRadarr() {
        let profile = ArrServiceProfile(displayName: "X", hostURL: "http://x", serviceType: .radarr)
        #expect(profile.resolvedServiceType == .radarr)
    }

    @Test func resolvedServiceTypeProwlarr() {
        let profile = ArrServiceProfile(displayName: "X", hostURL: "http://x", serviceType: .prowlarr)
        #expect(profile.resolvedServiceType == .prowlarr)
    }

    @Test func resolvedServiceTypeNilForUnknownString() {
        let profile = ArrServiceProfile(displayName: "X", hostURL: "http://x", serviceType: .sonarr)
        profile.serviceType = "unknown_service"
        #expect(profile.resolvedServiceType == nil)
    }

    @Test func allowsUntrustedTLSDefaultsFalse() {
        let profile = ArrServiceProfile(displayName: "X", hostURL: "http://x", serviceType: .sonarr)
        #expect(profile.allowsUntrustedTLS == false)
    }

    @Test func allowsUntrustedTLSTrueWhenExplicitlySet() {
        let profile = ArrServiceProfile(
            displayName: "X",
            hostURL: "https://x",
            serviceType: .sonarr,
    ...
    @Test func allowsUntrustedTLSSetterUpdatesValue() {
        let profile = ArrServiceProfile(displayName: "X", hostURL: "http://x", serviceType: .radarr)
        profile.allowsUntrustedTLS = true
        #expect(profile.allowsUntrustedTLS == true)
        profile.allowsUntrustedTLS = false
        #expect(profile.allowsUntrustedTLS == false)
    }

    @Test func initStoresHostURL() {
        let url = "http://192.168.1.100:8989"
        let profile = ArrServiceProfile(displayName: "Sonarr", hostURL: url, serviceType: .sonarr)
        #expect(profile.hostURL == url)
    }

    @Test func initStoresDisplayName() {
        let profile = ArrServiceProfile(displayName: "My Radarr", hostURL: "http://x", serviceType: .radarr)
        #expect(profile.displayName == "My Radarr")
    }

    @Test func initLastSyncedIsNil() {
        let profile = ArrServiceProfile(displayName: "X", hostURL: "http://x", serviceType: .sonarr)
        #expect(profile.lastSynced == nil)
    }

    @Test func initApiVersionIsNil() {
        let profile = ArrServiceProfile(displayName: "X", hostURL: "http://x", serviceType: .sonarr)
        #expect(profile.apiVersion == nil)
    }
}

// MARK: - ArrSetupViewModel Validation Tests

@MainActor
struct ArrSetupViewModelValidationTests {

    private func makeViewModel() -> ArrSetupViewModel {
        ArrSetupViewModel(serviceManager: ArrServiceManager())
    }

    @Test func initialStateHasEmptyFields() {
        let vm = makeViewModel()
        #expect(vm.hostURL == "")
        #expect(vm.apiKey == "")
        #expect(vm.displayName == "")
        #expect(vm.serviceType == .sonarr)
        #expect(vm.allowsUntrustedTLS == false)
        #expect(vm.isValidating == false)
        #expect(vm.validationError == nil)
        #expect(vm.validatedStatus == nil)
    }

    @Test func validateAndSaveReturnsFalseForEmptyURL() async throws {
        ...
        let result = await vm.validateAndSave(modelContext: context)
        #expect(result == false)
        #expect(vm.validationError == "Server URL is required.")
    }

    @Test func validateAndSaveReturnsFalseForWhitespaceOnlyURL() async throws {
        let schema = Schema([ArrServiceProfile.self])
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: schema, configurations: [config])
        let context = ModelContext(container)
    ...
    @Test func validateAndSaveReturnsFalseForEmptyAPIKey() async throws {
        let schema = Schema([ArrServiceProfile.self])
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: schema, configurations: [config])
        let context = ModelContext(container)
        ...
        let vm = makeViewModel()
        vm.hostURL = "http://192.168.1.1:8989"
        vm.apiKey = ""

        let result = await vm.validateAndSave(modelContext: context)
        #expect(result == false)
        #expect(vm.validationError == "API key is required.")
    }

    @Test func validateAndSaveReturnsFalseForWhitespaceOnlyAPIKey() async throws {
        let schema = Schema([ArrServiceProfile.self])
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: schema, configurations: [config])
        let context = ModelContext(container)
    ...
    @Test func validateAndSaveIsValidatingFalseAfterEmptyURL() async throws {
        let schema = Schema([ArrServiceProfile.self])
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: schema, configurations: [config])
        let context = ModelContext(container)
        ...
        _ = await vm.validateAndSave(modelContext: context)
        // isValidating should not be left as true since we returned early
        #expect(vm.isValidating == false)
    }

    @Test func serviceTypeDefaultIsSonarr() {
        let vm = makeViewModel()
        #expect(vm.serviceType == .sonarr)
    }

    @Test func serviceTypeCanBeChanged() {
        let vm = makeViewModel()
        vm.serviceType = .radarr
        #expect(vm.serviceType == .radarr)
        vm.serviceType = .prowlarr
        #expect(vm.serviceType == .prowlarr)
    }
}

// MARK: - ArrAPIClient Additional URL Tests

struct ArrAPIClientAdditionalTests {

    @Test func initPreservesSubdirectoryPath() async {
        // If user provides a base URL with a subpath (e.g. behind a reverse proxy)
        // The client should strip trailing slash, not alter a path
        let client = ArrAPIClient(baseURL: "http://localhost:8989", apiKey: "key")
        let baseURL = await client.baseURL
        #expect(baseURL == "http://localhost:8989")
    }

    @Test func initHandlesHTTPS() async {
        let client = ArrAPIClient(baseURL: "https://myserver.example.com", apiKey: "key")
        let baseURL = await client.baseURL
        #expect(baseURL == "https://myserver.example.com")
    }

    @Test func initHandlesPortInURL() async {
        let client = ArrAPIClient(baseURL: "http://192.168.1.1:7878/", apiKey: "testkey")
        let baseURL = await client.baseURL
        #expect(baseURL == "http://192.168.1.1:7878")
    }

    @Test func initHandlesMultipleTrailingSlashes() async {
        // Only one trailing slash should be stripped by the init logic
        // "http://localhost/" → "http://localhost"
        let client = ArrAPIClient(baseURL: "http://localhost/", apiKey: "key")
        let baseURL = await client.baseURL
        #expect(!baseURL.hasSuffix("/"))
    }

    @Test func defaultPageSizeIsPositive() {
        #expect(ArrAPIClient.defaultPageSize > 0)
    }
}

// MARK: - ArrBlocklistView BlocklistScope Tests

struct ArrBlocklistViewScopeTests {

    @Test func allScopeRawValue() {
        let scope = ArrBlocklistView.BlocklistScope.all
        #expect(scope.rawValue == "All")
    }

    @Test func seriesScopeRawValue() {
        let scope = ArrBlocklistView.BlocklistScope.series
        #expect(scope.rawValue == "Series")
    }

    @Test func moviesScopeRawValue() {
        let scope = ArrBlocklistView.BlocklistScope.movies
        #expect(scope.rawValue == "Movies")
    }

    @Test func scopeIdEqualsRawValue() {
        for scope in ArrBlocklistView.BlocklistScope.allCases {
            #expect(scope.id == scope.rawValue)
        }
    }

    @Test func allCasesCount() {
        #expect(ArrBlocklistView.BlocklistScope.allCases.count == 3)
    }

    @Test func initFromRawValueAll() {
        let scope = ArrBlocklistView.BlocklistScope(rawValue: "All")
        #expect(scope == .all)
    }

    @Test func initFromRawValueSeries() {
        let scope = ArrBlocklistView.BlocklistScope(rawValue: "Series")
        #expect(scope == .series)
    }

    @Test func initFromRawValueMovies() {
        let scope = ArrBlocklistView.BlocklistScope(rawValue: "Movies")
        #expect(scope == .movies)
    }

    @Test func initFromRawValueInvalidIsNil() {
        let scope = ArrBlocklistView.BlocklistScope(rawValue: "unknown")
        #expect(scope == nil)
    }
}

// MARK: - ArrBlocklistView BlocklistEntry Tests

struct ArrBlocklistEntryTests {

    private func makeItem(id: Int, sourceTitle: String? = nil) -> ArrBlocklistItem {
        let json: [String: Any] = ["id": id]
        let data = try! JSONSerialization.data(withJSONObject: json)
        return try! JSONDecoder().decode(ArrBlocklistItem.self, from: data)
    }

    @Test func entryIDCombinesSourceAndItemID() {
        let item = makeItem(id: 42)
        let entry = ArrBlocklistView.BlocklistEntry(item: item, source: .sonarr)
        #expect(entry.id == "sonarr-42")
    }

    @Test func entryIDForRadarr() {
        let item = makeItem(id: 7)
        let entry = ArrBlocklistView.BlocklistEntry(item: item, source: .radarr)
        #expect(entry.id == "radarr-7")
    }

    @Test func entryIDForProwlarr() {
        let item = makeItem(id: 99)
        let entry = ArrBlocklistView.BlocklistEntry(item: item, source: .prowlarr)
        #expect(entry.id == "prowlarr-99")
    }

    @Test func entryIDIsStableForSameInput() {
        let item = makeItem(id: 100)
        let entry1 = ArrBlocklistView.BlocklistEntry(item: item, source: .sonarr)
        let entry2 = ArrBlocklistView.BlocklistEntry(item: item, source: .sonarr)
        #expect(entry1.id == entry2.id)
    }

    @Test func entryIDDiffersForDifferentSources() {
        let item = makeItem(id: 5)
        let sonarrEntry = ArrBlocklistView.BlocklistEntry(item: item, source: .sonarr)
        let radarrEntry = ArrBlocklistView.BlocklistEntry(item: item, source: .radarr)
        #expect(sonarrEntry.id != radarrEntry.id)
    }
}

// MARK: - ArrCalendarViewModel Initial State Tests

@MainActor
struct ArrCalendarViewModelTests {

    @Test func initialIsLoadingInitialTrue() {
        let manager = ArrServiceManager()
        let vm = ArrCalendarViewModel(serviceManager: manager)
        #expect(vm.isLoadingInitial == true)
    }

    @Test func initialIsLoadingMoreFalse() {
        let manager = ArrServiceManager()
        let vm = ArrCalendarViewModel(serviceManager: manager)
        #expect(vm.isLoadingMore == false)
    }

    @Test func initialIsLoadingEarlierFalse() {
        let manager = ArrServiceManager()
        let vm = ArrCalendarViewModel(serviceManager: manager)
        #expect(vm.isLoadingEarlier == false)
    }

    @Test func initialSonarrSeriesEmpty() {
        let manager = ArrServiceManager()
        let vm = ArrCalendarViewModel(serviceManager: manager)
        #expect(vm.sonarrSeries.isEmpty)
    }

    @Test func initialRadarrMoviesEmpty() {
        let manager = ArrServiceManager()
        let vm = ArrCalendarViewModel(serviceManager: manager)
        #expect(vm.radarrMovies.isEmpty)
    }

    @Test func scrollIDInitializedToToday() {
        let manager = ArrServiceManager()
        let vm = ArrCalendarViewModel(serviceManager: manager)
        // scrollID should be set to start of today
        let today = Calendar.current.startOfDay(for: .now)
        #expect(vm.scrollID == today)
    }
}

// MARK: - ArrBlocklistItem Codable Tests

struct ArrBlocklistItemCodableTests {

    @Test func decodeFullBlocklistItem() throws {
        let json = #"""
        {
            "id": 10,
            "seriesId": 2,
        ...
        """#
        let data = json.data(using: .utf8)!
        let item = try JSONDecoder().decode(ArrBlocklistItem.self, from: data)
        #expect(item.id == 10)
        #expect(item.seriesId == 2)
        #expect(item.movieId == nil)
        #expect(item.episodeIds == [100, 101])
        #expect(item.sourceTitle == "Show.S01E01.HDTV")
        #expect(item.indexer == "MyIndexer")
        #expect(item.message == "Corrupt download")
        #expect(item.date == "2024-03-15T12:00:00Z")
        #expect(item.quality?.quality?.name == "HDTV-720p")
    }

    @Test func decodeMinimalBlocklistItem() throws {
        let json = #"{"id": 99}"#
        let data = json.data(using: .utf8)!
        let item = try JSONDecoder().decode(ArrBlocklistItem.self, from: data)
        #expect(item.id == 99)
    ...
    @Test func decodeMovieBlocklistItem() throws {
        let json = #"""
        {
            "id": 5,
            "movieId": 42,
            "sourceTitle": "Movie.2024.BluRay",
        ...
        let item = try JSONDecoder().decode(ArrBlocklistItem.self, from: data)
        #expect(item.id == 5)
        #expect(item.movieId == 42)
        #expect(item.seriesId == nil)
    }
}

// MARK: - ArrHistoryRecord Tests

struct ArrHistoryRecordTests {
    ...
    @Test func decodeAllFields() throws {
        ...
        """#
        let data = json.data(using: .utf8)!
        let record = try JSONDecoder().decode(ArrHistoryRecord.self, from: data)
        #expect(record.id == 7)
        #expect(record.eventType == "grabbed")
        #expect(record.date == "2024-05-10T08:00:00Z")
        #expect(record.sourceTitle == "Show.S03E05.720p")
        #expect(record.downloadId == "abc-123")
        #expect(record.seriesId == 10)
        #expect(record.episodeId == 55)
        #expect(record.movieId == nil)
    }

    @Test func decodeMinimalHistoryRecord() throws {
        let json = #"{"id": 1}"#
        let data = json.data(using: .utf8)!
        let record = try JSONDecoder().decode(ArrHistoryRecord.self, from: data)
        #expect(record.id == 1)
        #expect(record.eventType == nil)
        #expect(record.sourceTitle == nil)
    }

    @Test func decodeRadarrHistoryRecord() throws {
        let json = #"""
        {
            "id": 20,
            "eventType": "downloadFolderImported",
            "movieId": 15,
            "sourceTitle": "GreatMovie.2023.BluRay"
        }
        """#
        let data = json.data(using: .utf8)!
        let record = try JSONDecoder().decode(ArrHistoryRecord.self, from: data)
        #expect(record.movieId == 15)
        #expect(record.seriesId == nil)
    }
}

// MARK: - ArrCommand Tests

struct ArrCommandTests {

    @Test func decodeCommand() throws {
        let json = #"""
        {
            "id": 1,
            "name": "RefreshSeries",
        ...
        """#
        let data = json.data(using: .utf8)!
        let cmd = try JSONDecoder().decode(ArrCommand.self, from: data)
        #expect(cmd.id == 1)
        #expect(cmd.name == "RefreshSeries")
        #expect(cmd.status == "queued")
        #expect(cmd.trigger == "manual")
    }

    @Test func decodeMinimalCommand() throws {
        let json = #"{}"#
        let data = json.data(using: .utf8)!
        let cmd = try JSONDecoder().decode(ArrCommand.self, from: data)
        #expect(cmd.id == nil)
...
// MARK: - ArrRelease Boundary Tests

struct ArrReleaseBoundaryTests {

    private func makeRelease(json: [String: Any]) -> ArrRelease {
        let data = try! JSONSerialization.data(withJSONObject: json)
        return try! JSONDecoder().decode(ArrRelease.self, from: data)
    }

    @Test func ageDescriptionExactly24Hours() {
        // 24 hours → 1 day
        let release = makeRelease(json: ["ageHours": 24.0])
        #expect(release.ageDescription == "1d")
    }

    @Test func ageDescriptionJustUnder24Hours() {
        // 23 hours → "23h"
        let release = makeRelease(json: ["ageHours": 23.0])
        #expect(release.ageDescription == "23h")
    }

    @Test func ageDescriptionAgeHoursTakesPriorityOverAge() {
        // ageHours (non-zero) wins over age
        let release = makeRelease(json: ["ageHours": 5.0, "age": 3])
        #expect(release.ageDescription == "5h")
    }

    @Test func ageDescriptionAgeTakesPriorityOverAgeMinutes() {
        // ageHours is zero/nil, age (non-zero) wins over ageMinutes
        let release = makeRelease(json: ["age": 2, "ageMinutes": 90.0])
        #expect(release.ageDescription == "2d")
    }

    @Test func idWithNegativeIndexerIDFallback() {
        // No indexerId in JSON → indexerId is nil → id uses -1
        let json: [String: Any] = ["guid": "test-guid"]
        let data = try! JSONSerialization.data(withJSONObject: json)
        let release = try! JSONDecoder().decode(ArrRelease.self, from: data)
        #expect(release.id == "test-guid|-1")
    }

    @Test func canGrabTrueWhenApprovedTrueAndRejectedFalse() {
        let release = makeRelease(json: ["approved": true, "rejected": false, "downloadAllowed": true])
        #expect(release.canGrab == true)
    }

    @Test func canGrabFalseWhenBothApprovedAndTemporarilyRejected() {
        let release = makeRelease(json: ["approved": true, "temporarilyRejected": true])
        #expect(release.canGrab == false)
    }

    @Test func qualityNameEmptyStringFallback() {
        // quality.quality.name is empty string
        let json: [String: Any] = ["quality": ["quality": ["name": ""]]]
        let data = try! JSONSerialization.data(withJSONObject: json)
        let release = try! JSONDecoder().decode(ArrRelease.self, from: data)
        // Empty string should still be returned (not the fallback)
        #expect(release.qualityName == "")
    }
}

// MARK: - ArrServiceManager calendarViewModel Tests

@MainActor
struct ArrServiceManagerCalendarTests {

    @Test func calendarViewModelNotNilAfterInit() {
        let manager = ArrServiceManager()
        // calendarViewModel is initialized with a lazy approach; accessing it should not crash
        let vm = manager.calendarViewModel
        #expect(vm != nil)
    }

    @Test func calendarViewModelIsLoadingInitialTrue() {
        let manager = ArrServiceManager()
        let vm = manager.calendarViewModel!
        #expect(vm.isLoadingInitial == true)
    }
...
// MARK: - ArrQueueItem Additional Tests

struct ArrQueueItemAdditionalTests {

    private func makeItem(json: [String: Any]) -> ArrQueueItem {
        let data = try! JSONSerialization.data(withJSONObject: json)
        return try! JSONDecoder().decode(ArrQueueItem.self, from: data)
    }

    @Test func estimatedCompletionTimeDecoded() throws {
        let json = #"""
        {
            "id": 1,
            "estimatedCompletionTime": "2024-06-01T10:30:00Z"
        }
        """#
        let data = json.data(using: .utf8)!
        let item = try JSONDecoder().decode(ArrQueueItem.self, from: data)
        #expect(item.estimatedCompletionTime == "2024-06-01T10:30:00Z")
    }

    @Test func downloadIdDecoded() throws {
        let json = #"{"id": 1, "downloadId": "torrent-hash-abc"}"#
        let data = json.data(using: .utf8)!
        let item = try JSONDecoder().decode(ArrQueueItem.self, from: data)
        #expect(item.downloadId == "torrent-hash-abc")
    }

    @Test func normalizedStateWithLeadingWhitespace() {
        let item = makeItem(json: ["id": 1, "status": "  Downloading  "])
        #expect(item.normalizedState == "downloading")
    }

    @Test func isImportIssueForImportingState() {
        // "importing" is not directly listed but "importPending" is - check importing is not matched
        let item = makeItem(json: ["id": 1, "trackedDownloadState": "importing"])
        // "importing" should not trigger isImportIssueQueueItem
        #expect(item.isImportIssueQueueItem == false)
    }

    @Test func progressWithLargeNumbers() {
        let item = makeItem(json: ["id": 1, "size": 107374182400.0, "sizeleft": 53687091200.0])
        #expect(abs(item.progress - 0.5) < 0.001)
    }
}