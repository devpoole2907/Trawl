...
|---|---|---|
| `Trawl/ArrStack/ArrReleaseActionContent.swift` | `TrawlTests/ArrAPIClientContractTests.swift`; `TrawlTests/ArrClientLifecycleTests.swift` | Request/response and stale-client routing are covered. A full UI release-selection/download journey is still required. |
| `Trawl/ArrStack/SonarrSeriesSearchViews.swift` | `TrawlTests/ArrAPIClientContractTests.swift`; `TrawlTests/ArrClientLifecycleTests.swift`; `TrawlUITests/ArrSearchAddJourneyUITests.swift` | New-series add is covered. Existing-series Automatic Search and Interactive Search → Download Release are not yet covered end to end. |
| `Trawl/ArrStack/RadarrMovieSearchViews.swift` | `TrawlTests/ArrAPIClientContractTests.swift`; `TrawlTests/ArrClientLifecycleTests.swift`; `TrawlUITests/RadarrJourneyUITests.swift` | Detail rendering and client contracts are covered. Add-new movie and Automatic/Interactive Search journeys remain open. |
| `Trawl/ArrStack/SonarrSeriesDetailView.swift` | `TrawlUITests/SonarrConnectedJourneyUITests.swift`; `TrawlUITests/ArrRepointJourneyUITests.swift`; `TrawlUITests/ArrInstanceSwitchJourneyUITests.swift`; `TrawlUITests/ArrSearchAddJourneyUITests.swift` | Library/repoint/switch/add assembly is covered; release-search actions need the journeys noted above. |
| `Trawl/ArrStack/RadarrMovieDetailView.swift` | `TrawlUITests/RadarrJourneyUITests.swift`; `TrawlTests/ArrAPIClientContractTests.swift`; `TrawlTests/ArrClientLifecycleTests.swift` | Real detail rendering and monitored mutation are covered; release-search actions need the journeys noted above. |
