...
| Issues, Users, and Jellyfin Libraries as native split-view columns — `RootTab.wantsDetailColumn`, `ContentView`'s `contentColumn`/`detailColumn` switches, `Trawl/SeerrStack/SeerrIssueBrowserState.swift`, `Trawl/Views/UnifiedUserBrowserState.swift`, `Trawl/JellyfinStack/JellyfinLibraryBrowserState.swift`, `Trawl/SeerrStack/SeerrIssueListView.swift`, `Trawl/Views/UnifiedUserListView.swift`, `Trawl/JellyfinStack/JellyfinLibrariesView.swift` | `TrawlTests/SeerrIssueListViewModelTests.swift`; `TrawlTests/SeerrIssueDetailViewModelTests.swift`; `TrawlUITests/JellyfinUserEditorJourneyUITests.swift`; `TrawlUITests/JellyfinLibrariesJourneyUITests.swift`; `TrawlUITests/SeerrJellyfinUserImportJourneyUITests.swift`; `TrawlUITests/IPadSidebarJourneyUITests.swift` | Screens were plain full-width lists whose rows were `NavigationLink { destination } label: { row }`; in a macOS `List` that form collapses the row to a fixed height, and empty states using standalone `ContentUnavailableView` broke safe-area insets and background gradients. They now use `TrawlListDetailPanes` like Quality Profiles and Requests, which also moves them from `twoColumnLayout` to `threeColumnLayout`. **The row is only a selection when a detail pane is beside it** (`hasDetailPane`), and still pushes at compact width. The view model is hoisted into the browser state because each column instantiates the screen separately: held per column, updates in the pane would not reach the row beside it, and the detail column would refetch. The detail column is barred from loading (`guard sidebarColumn != .detail`), matching `SeerrDashboardView`. |
