...
| Users and Jellyfin Libraries as native split-view columns — `RootTab.wantsDetailColumn`, `ContentView`'s `contentColumn`/`detailColumn` switches, `Trawl/Views/UnifiedUserBrowserState.swift`, `Trawl/JellyfinStack/JellyfinLibraryBrowserState.swift`, `Trawl/Views/UnifiedUserListView.swift`, `Trawl/JellyfinStack/JellyfinLibrariesView.swift` | `TrawlUITests/JellyfinUserEditorJourneyUITests.swift`; `TrawlUITests/JellyfinLibrariesJourneyUITests.swift`; `TrawlUITests/SeerrJellyfinUserImportJourneyUITests.swift`; `TrawlUITests/IPadSidebarJourneyUITests.swift` | Both screens were plain full-width lists whose rows were `NavigationLink { destination } label: { row }`; in a macOS `List` that form collapses the row to a fixed height, so a two-line row overflowed its own separator. They now use `TrawlListDetailPanes` like Quality Profiles and Requests, which also moves them from `twoColumnLayout` to `threeColumnLayout`. **The row is only a selection when a detail pane is beside it** (`hasDetailPane`), and still pushes at compact width — the three journeys above tap a row and then assert on the pushed screen's navigation bar, so they exercise the push path on iPhone and the pane path on iPad. The view model is hoisted into the browser state because each column instantiates the screen separately: held per column, a rename in the pane would not reach the row beside it, and the detail column would refetch. The detail column is barred from loading (`guard sidebarColumn != .detail`), matching `SeerrDashboardView`. |
