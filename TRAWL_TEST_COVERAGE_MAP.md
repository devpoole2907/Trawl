...
|---|---|---|
| `Trawl/ArrStack/AppIntents/` — Siri/Shortcuts library checks, searches, adds, entities and registration | `TrawlTests/ArrIntentSupportTests.swift`; `TrawlTests/ArrAddIntentDuplicateCheckTests.swift` | Entity identifiers and safe add defaults are covered. The real intent execution path is covered against loopback Radarr/Sonarr servers for conversational availability answers, exact downloaded/monitored language, absence, failed reads, duplicate-check failures, and successful empty-library adds. App Shortcut discovery and voice-language routing remain physical-device release smoke-test work. |
