# Trawl reliability audit — archive

**Frozen 28 August 2026.** This file is the full historical narrative of the reliability
and test audit: the original 22 August audit, eleven implementation tranches, and every
defect write-up in the order it was found.

**Do not read this file to learn the current state of the project — it will mislead you.**
Much of it describes conditions that no longer hold. To pick one example, the original
audit records 2.66% production coverage, a `TrawlMac` target that does not compile, and
no test plan; all three were fixed within days. Read
[`TRAWL_RELIABILITY_TEST_AUDIT.md`](TRAWL_RELIABILITY_TEST_AUDIT.md) for what is true now.

This file exists so evidence and reasoning are not lost. Use it when you need the detail
behind a specific finding — how a defect was proved, what the negative control showed —
after the working document has told you the finding matters.
...
- **M-03 remains partially fixed:** a deterministic qBittorrent 403 reauthentication contract still needs an injectable credential/reauthentication seam.
- ~~**`ArrSetupViewModel` has no unit-test file at all.**~~ **Closed:** `TrawlTests/ArrSetupViewModelTests.swift` now covers the add path — the three pre-flight guards (each proven to reach no socket against a live server), a rejected key leaving no profile and no Keychain entry, a successful add persisting the exact host/key/version/instance name, and the Prowlarr singleton branch. Two negative controls were run against production: removing the Prowlarr adoption branch fails that test at three assertions, and clearing `ArrAPIClient`'s `unauthorizedStatusCodes` fails the rejected-key test on its exact-message assertion. What remains unexercised inside `validateAndSave` is the failure unwinding itself — the `modelContext.rollback()`, the Keychain delete, and the cancellation restore all need a save or a cancellation to fail on purpose, which has no seam today.
- **The add-a-service path is uncovered, and it is not the same code as the edit path.** Adding inserts a new profile and, on failure or cancellation, unwinds through `modelContext.rollback()` plus a Keychain delete; editing instead restores each field and rewrites the original key. Adding Prowlarr when one already exists silently *updates the existing profile* and disables every other Prowlarr profile (`ArrSetupViewModel.swift:87-90`). The service-type picker renders only in this flow (`initialServiceType == nil && existingProfile == nil`). None of that has a test.
